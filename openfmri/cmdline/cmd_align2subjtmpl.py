...
def align_file(wf, label, subj, subj_sink, data, reference,
               tmpl_brain_mask, fgthresh):
    subj_aligned = pe.Node(
            name='sub%.3i_aligned' % subj,
            interface=util.Merge(len(data.outputs.out_paths)))
    # do the same thing for each input file
    for i, input in enumerate(sorted(data.outputs.out_paths)):
...
def run(args):
    ...
    for subj in subjects:
        ...
        wf = align_file(wf, label, subj, subj_sink, result, reference,
                        tmpl_brain_mask, fgthresh)
    return wf
