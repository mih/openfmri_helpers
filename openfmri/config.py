...
class ConfigManager(SafeConfigParser):
    """Central configuration registry for openfmri.

    The purpose of this class is to collect all configurable settings
    used by various parts of openfmri. It is fairly simple and does
    ...
    1. '/etc/openfmri/openfmri.cfg'
    2. 'openfmri/config' in all directories defined by $XDG_CONFIG_DIRS
       (by default: /etc/xdg/)
    3. 'openfmri.cfg' in $XDG_CONFIG_HOME (by default: ~/.config/)
    4. 'openfmri.cfg' in the current directory
    ...
    variables have to start with `OPENFMRI_` following by the an
    ...
    However, `OPENFMRI_VERBOSE_OUTPUT=stdout` becomes::

        [verbose]
        output = stdout

    Any length of variable name as allowed, e.g. OPENFMRI_SEC1_LONG_NAME=1
    becomes::
    ...
    accordingly). Finally, the content of any `OPENFMRI_*` environment variables
    ...
    def reload(self, filenames=None):
        ...
        cfg_file_candidates = [
            # shipped-with config
            opj(os.path.dirname(__file__), 'openfmri.cfg'),
            # system config
            '/etc/openfmri/openfmri.cfg']
        # XDG system config
        cfg_file_candidates += [opj(b, 'openfmri', 'config')
        ...
        if os.path.isabs(home_cfg_base_path):
            cfg_file_candidates.append(opj(home_cfg_base_path, 'openfmri.cfg'))
        # current dir config
        cfg_file_candidates.append('openfmri.cfg')
        # runtime config
        ...
        for var in [v for v in os.environ.keys() if v.startswith('OPENFMRI_')]:
            # strip leading 'OPENFMRI_' and lower case entries
            svar = var[10:].lower()
